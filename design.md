...
- **Hero entrance:** on first paint the headline reveals with a typed-in effect (real text is present in the DOM throughout for SEO/screen readers; the animation is a decorative overlay). The greeting line, subtitle, and buttons fade up on a fixed short stagger (0/100/200ms) that starts immediately on mount — deliberately *not* gated on the typing animation finishing, so they're never left invisible while the JS bundle hydrates or the typewriter runs (an earlier version tied this to typing completion; a 2026-07-23 audit found it left above-the-fold content invisible for several seconds and excluded from LCP). Reduced motion shows the final state immediately, no delay.
